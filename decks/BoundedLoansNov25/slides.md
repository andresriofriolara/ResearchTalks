...
&=
\underbrace{
  \underbrace{\,w(p_t)\!\big[(c_t^{\mathrm{repay}}-r_t)_{+}^{\alpha_v}
  - \lambda\,(r_t-c_t^{\mathrm{repay}})_{+}^{\beta_v}\big]\,}_{\text{repay branch}}
  \;+\;
  \underbrace{\,\bigl(1-w(p_t)\bigr)\!\big[(c_t^{\mathrm{default}}-r_t)_{+}^{\alpha_v}
  - \lambda\,(r_t-c_t^{\mathrm{default}})_{+}^{\beta_v}\big]\,}_{\text{default branch}}
...
\underbrace{
  ...
  \mathbb{E}\!\left[
    \underbrace{\,w(p_\tau)\!\big[(c_\tau^{\mathrm{repay}}-r_\tau)_{+}^{\alpha_v}
    - \lambda\,(r_\tau-c_\tau^{\mathrm{repay}})_{+}^{\beta_v}\big]\,}_{\text{repay at }\tau}
    \;+\;
    \underbrace{\,\bigl(1-w(p_\tau)\bigr)\!\big[(c_\tau^{\mathrm{default}}-r_\tau)_{+}^{\alpha_v}
    - \lambda\,(r_\tau-c_\tau^{\mathrm{default}})_{+}^{\beta_v}\big]\,}_{\text{default at }\tau}
  \right]
