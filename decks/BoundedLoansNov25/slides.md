...
$$
U_t^{\mathrm{within}} = w(p_t)\,(c_t^{R}-r_t)^{\alpha} - \lambda\,(r_t-c_t^{R})^{\beta} + \bigl(1-w(p_t)\bigr)\,(c_t^{D}-r_t)^{\alpha} - \lambda\,(r_t-c_t^{D})^{\beta}
$$

$$
U_t^{\mathrm{future}} = \beta \sum_{\tau=t+1}^{T} \delta^{\tau-t}\, \mathbb{E}\!\left[ w(p_\tau)\,(c_\tau^{R}-r_\tau)^{\alpha} - \lambda\,(r_\tau-c_\tau^{R})^{\beta} + \bigl(1-w(p_\tau)\bigr)\,(c_\tau^{D}-r_\tau)^{\alpha} - \lambda\,(r_\tau-c_\tau^{D})^{\beta} \right]
$$
