...
&=
\underbrace{
  \underbrace{w(p_t)\big[(c_t^{\text{repay}}-r_t)_{+}^{\alpha_v}\big]}_{\text{repay branch}}
  - \lambda\,(r_t-c_t^{\text{repay}})_{+}^{\beta_v}
  + \underbrace{\bigl(1-w(p_t)\bigr)\big[(c_t^{\text{default}}-r_t)_{+}^{\alpha_v}\big]}_{\text{default branch}}
  - \lambda\,(r_t-c_t^{\text{default}})_{+}^{\beta_v}
}_{
  \begin{array}{c}
...
\underbrace{
  \beta \sum_{\tau=t+1}^{T} \delta^{\tau-t}\,
    \mathbb{E}\left[
      w(p_\tau)\big[(c_\tau^{\text{repay}}-r_\tau)_{+}^{\alpha_v}\big]
      - \lambda\,(r_\tau-c_\tau^{\text{repay}})_{+}^{\beta_v}
      + \bigl(1-w(p_\tau)\bigr)\big[(c_\tau^{\text{default}}-r_\tau)_{+}^{\alpha_v}\big]
      - \lambda\,(r_\tau-c_\tau^{\text{default}})_{+}^{\beta_v}
    \right]
