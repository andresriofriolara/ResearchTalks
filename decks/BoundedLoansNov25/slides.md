...
$$
U_t^{\mathrm{future}}=\beta\sum_{\tau=t+1}^{T}\delta^{\tau-t}\,E\!\left[w(p_\tau)(c_\tau^{R}-r_\tau)^{\alpha}-\lambda(r_\tau-c_\tau^{R})^{\beta}+(1-w(p_\tau))(c_\tau^{D}-r_\tau)^{\alpha}-\lambda(r_\tau-c_\tau^{D})^{\beta}\right]
$$
