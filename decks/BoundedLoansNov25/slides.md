...
$$
U_t^{\text{within}} = w(p_t)\,(c_t^{repay}-r_t)_{+}^{\alpha}- \lambda\,(r_t-c_t^{repay})_{+}^{\beta}+ (1-w(p_t))\,(c_t^{default}-r_t)_{+}^{\alpha}- \lambda\,(r_t-c_t^{default})_{+}^{\beta}
$$

$$
U_t^{\text{future}} = \beta \sum_{\tau=t+1}^{T} \delta^{\tau-t}\E\Big[ w(p_\tau)\,(c_\tau^{repay}-r_\tau)_{+}^{\alpha} - \lambda\,(r_\tau-c_\tau^{repay})_{+}^{\beta} + (1-w(p_\tau))\,(c_\tau^{default}-r_\tau)_{+}^{\alpha} - \lambda\,(r_\tau-c_\tau^{default})_{+}^{\beta} \Big]
$$
